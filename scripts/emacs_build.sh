...
EMACS_CONFIGURE_SCRIPT="$HOME/.config/jconfig/emacs-configure.sh"
...
# JCONFIG_EMACS_AUTO_UPDATE_MINOR.
emacs_load_settings() {
...
# Generate the default configure script if it doesn't exist.
# The script contains a comprehensive set of build flags.
emacs_generate_configure_script() {
    if [[ -f "$EMACS_CONFIGURE_SCRIPT" ]]; then
        return 0
    fi

    mkdir -p "$(dirname "$EMACS_CONFIGURE_SCRIPT")"
    cat > "$EMACS_CONFIGURE_SCRIPT" <<'SCRIPT'
#!/usr/bin/env bash
# Emacs build configuration script -- generated by jconfig.
# Edit this file to customize your emacs build flags.
# This script is called with two arguments:
#   $1 = source directory (containing ./configure)
#   $2 = install prefix (e.g., ~/.local/emacs)
#
# The default flags below provide a full-featured emacs build with
# native compilation, tree-sitter, and all common UI/image support.
# Comment out or remove flags you don't need.
...
./configure \
    ...
    "$@"
SCRIPT
    chmod +x "$EMACS_CONFIGURE_SCRIPT"
    echo "Generated build configuration at $EMACS_CONFIGURE_SCRIPT"
}

# Ensure the configure script exists and offer the user a chance to edit it.
# Called before the build pipeline runs.
emacs_prepare_configure_script() {
    local is_new=false
    if [[ ! -f "$EMACS_CONFIGURE_SCRIPT" ]]; then
        emacs_generate_configure_script
        is_new=true
    ...
    echo ""
    echo "Build configuration script: $EMACS_CONFIGURE_SCRIPT"
    if [[ "$is_new" == "true" ]]; then
        echo "A default build script has been generated with recommended flags."
    fi

    local customize
    read -r -p "Would you like to review/edit the build flags before building? (y/n) [n]: " customize
    if [[ "${customize:-n}" =~ ^[Yy] ]]; then
        echo ""
        echo "Current build configuration:"
        echo "─────────────────────────────"
        cat "$EMACS_CONFIGURE_SCRIPT"
        echo "─────────────────────────────"
        echo ""
        echo "Edit the file at: $EMACS_CONFIGURE_SCRIPT"
        read -r -p "Press Enter when you're done editing to continue the build... "
    fi
}
...
# Run ./configure via the user's configure script.
# $1: version string
...
emacs_configure_source() {
    ...
    echo "  script: $EMACS_CONFIGURE_SCRIPT"

    if ! "$EMACS_CONFIGURE_SCRIPT" "$src_dir" "$EMACS_INSTALL_PREFIX"; then
        echo ""
        ...
        echo "You can edit $EMACS_CONFIGURE_SCRIPT to adjust flags and re-run."
        return 1
    fi
