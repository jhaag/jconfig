...
- [[#tasks-][Tasks]]
  - [[#reconcile-legacy-packages-and-languages-with-initorg][Reconcile legacy ~packages/~ and ~languages/~ with ~init.org~]]
...
** TODO Reconcile legacy ~packages/~ and ~languages/~ with ~init.org~

*** Background: two config eras

This file (~init.org~) is the *current source of truth*. All ~emacs-lisp~ code
blocks tangle to [[file:tangles/init.el][tangles/init.el]], which is loaded by ~~/.emacs~ via
~j/config/load-conf-dot-org~.

There is also a *legacy system* consisting of hand-written ~.el~ files in
[[file:packages/][packages/]] and [[file:languages/][languages/]], loaded by [[file:packages.el][packages.el]] and [[file:languages.el][languages.el]] respectively.
These loader files are *never referenced* anywhere in the active config chain
and are effectively dead code — none of these files are loaded at runtime.

*** Current packages managed by ~init.org~

*Utility / UI*
| Package            | Notes                                      |
|--------------------+--------------------------------------------|
| agent-shell        | LLM agent interface (ACP)                  |
| buffer-move        | Window rearrangement helpers               |
| exec-path-from-shell | Sync ~$PATH~ from shell                  |
| git-modes          | ~.gitconfig~ / ~.gitignore~ major modes    |
| git-gutter+        | Diff indicators in fringe                  |
| markdown-mode      |                                            |
| multiple-cursors   |                                            |
| pdf-tools          |                                            |
| saveplace          | Remember buffer positions                  |
| selectric-mode     | Typewriter sounds                          |
| eterm-256color     | 256-color support for ~ansi-term~          |
| shell-pop          | Pop-up terminal (bound to ~<f1>~)          |
| smooth-scrolling   |                                            |
| solarized-theme    |                                            |
| symon              | System monitor in modeline                 |
| undo-tree          |                                            |
| xclip              | Sync kill-ring with system clipboard       |
| zone               | Screensaver                                |

*Completion / Navigation*
| Package            | Notes                                      |
|--------------------+--------------------------------------------|
| helm               | + ~helm-mode~ and ~20+~ sub-packages       |
| flycheck           | Loaded as helm-flycheck dependency         |
| company            | Loaded as helm-company dependency          |
...
| asm-mode           |                                            |
| proof-general      | Coq                                        |
| company-coq        | Coq completion                             |
| js2-mode           | JavaScript                                 |
| js2-refactor       |                                            |
| xref-js2           |                                            |
| nael               | Lean 4                                     |
| haskell-mode       | (via ~j-dot-org.org~ / languages/)         |
| rust-mode          |                                            |
| org-mode           | Extended config in ~j-dot-org.org~         |

*** Legacy ~.el~ files (orphaned, never loaded)

*packages/* — 16 files, none loaded at runtime:
| File                  | Status in ~packages.el~  | Also in ~init.org~? |
|-----------------------+--------------------------+---------------------|
| j-buffer-move.el      | active (not commented)   | YES — duplicate     |
| j-company.el          | commented out            | YES — duplicate     |
| j-git-gutter+.el      | active                   | YES — duplicate     |
| j-helm.el             | active                   | YES — duplicate     |
| j-multiple-cursors.el | active                   | YES — duplicate     |
| j-shell-pop.el        | active                   | YES — duplicate     |
| j-smooth-scrolling.el | active                   | YES — duplicate     |
| j-solarized.el        | active                   | YES — duplicate     |
| j-symon.el            | active                   | YES — duplicate     |
| j-undo-tree.el        | commented out            | YES — duplicate     |
| j-function-args.el    | commented out            | never migrated      |
| j-sr-speedbar.el      | commented out            | never migrated      |
| j-ssh.el              | commented out            | never migrated      |
| j-tramp.el            | commented out            | never migrated      |
| j-vagrant-tramp.el    | commented out            | never migrated      |
| j-wttrin.el           | commented out            | never migrated      |

*languages/* — 5 files, none loaded at runtime:
| File          | Also in ~init.org~? |
|---------------+---------------------|
| j-asm.el      | YES — duplicate     |
| j-haskell.el  | YES — duplicate     |
| j-js.el       | YES — duplicate     |
| j-org.el      | YES — duplicate     |
| j-rust.el     | YES — duplicate     |

*** Analysis

- 10 packages were fully migrated to ~init.org~ and their legacy ~.el~
  counterparts are now stale duplicates (configs have diverged — ~init.org~
  versions are more up-to-date).
- 5 language configs are likewise duplicated; ~init.org~ is authoritative.
- 6 packages (~function-args~, ~sr-speedbar~, ~ssh~, ~tramp~, ~vagrant-tramp~,
  ~wttrin~) exist only in legacy files and were never migrated; all are
  commented out, suggesting intentional disuse.
- ~packages.el~, ~languages.el~, ~packages/~, and ~languages/~ can be safely
  deleted once confirmed no longer needed.
...
:PROPERTIES:
:VISIBILITY: folded
:header-args:emacs-lisp: :tangle (concat (expand-file-name "~/jconfig/.emacs.d/tangles/tests/") (file-name-base) "-test.el")
:END:
...
#+BEGIN_SRC emacs-lisp
  (use-package buffer-move
    ...
    :bind (("C-x <up>"          . buf-move-up)
           ...
           ("C-`"               . other-window)
           ;; for windowless mode:
           ;; - C-` is sent as C-@ by default, which maps to
           ;;   ("C-{Spc, @}" . set-mark-command)
           ;;   ==> rebind C-@ to other-window and use C-c C-@ for set-mark-command
           ("C-@"               . other-window)
           ("C-c C-@"           . set-mark-command)
           ;; for consistency in windowed mode
           ("C-c C-SPC"         . set-mark-command)
          ))
#+END_SRC
...
(use-package helm-unicode
  :ensure t
  :bind* ("C-x 8 <return>" . helm-unicode))
#+END_SRC
** Language-Specific
