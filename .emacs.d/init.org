#+PROPERTY: header-args:emacs-lisp :tangle (concat (expand-file-name "~/jaspah/.emacs.d/tangles/") (file-name-base) ".el")
#+PROPERTY: header-args :mkdirp yes :comments no
#+STARTUP: indent overview

#+TITLE: My Emacs config file
#+AUTHOR: Jasper Haag
#+EMAIL: jhaag@alum.mit.edu
...
- [[#settings][Settings]]
  ...
  - [[#general][General]]
- [[#hooks][Hooks]]
  - [[#config-auto-tangle--daemon-restart][Config Auto-tangle & Daemon Restart]]
...
;;; jbootstrap --- Efficiently/lazily autotangle & load ~/jaspah/init.org. -*- lexical-binding: t -*-
;;; Commentary:
;;;   This is an auto-generated file, so PLEASE DON'T TOUCH ANYTHING!
;;;   If you want to make changes to the bootstrapping code, open
;;;   ~/jaspah/.emacs.d/init.org, C-x n s on [[Bootstrap Process]],
...
I use the [[https://www.masteringemacs.org/article/spotlight-use-package-a-declarative-configuration-tool][wonderful use-package]] (now distributed as part of emacs) to manage most of the packages in my installation. Since I'm using use-package, I need to do a bit of initialization in my init.el stub in order to get things bootstrapped.
...
Automatically check for and apply package updates weekly using [[https://github.com/rranelli/auto-package-update.el][auto-package-update]].
Prompts before updating, shows a preview of packages to be updated, displays results after completion,
and cleans up old package versions.
...
(defun j/config/load-conf-dot-org (filename)
  "Tangle all source blocks in FILENAME and load the resulting elisp.
Tangles unconditionally so every output target (main config, tests, etc.) is
always up-to-date.  The main output is expected at tangles/<basename>.el
alongside the org file."
  (let* ((filename (expand-file-name filename))
         (target (concat (file-name-directory filename)
                         "tangles/" (file-name-base filename) ".el")))
...
With the rest of the setup done, it is quite easy to load the rest of the configuration.

#+BEGIN_SRC emacs-lisp :tangle (expand-file-name "~/.emacs")
(j/config/load-conf-dot-org "~/jaspah/.emacs.d/init.org")
#+END_SRC
...
(setq custom-file "~/jaspah/.emacs.d/.emacs-custom.el")
(load custom-file)
#+END_SRC

#+INCLUDE: "~/jaspah/.emacs.d/.emacs-custom.el" src emacs-lisp
...
(eval-and-compile
  ...
  (defun j/frame-portrait-p (&optional frame)
    "Return non-nil if FRAME's current physical monitor is in portrait orientation.
FRAME defaults to the selected frame."
    (let* ((geometry (assoc 'geometry (frame-monitor-attributes frame)))
...
;; Ensure that pasting works with emacs -nw
...
- I want emacs to start maximized (see [[https://emacs.stackexchange.com/a/3008][this]] answer for details on my more complicated solution; I no longer just toggle the first frame that opens).
  #+BEGIN_SRC emacs-lisp
...
- I configure emacs to automatically sync my buffer unless I have unsaved changes.
...
- I ensure that if a file has a "Time-stamp: <>" at the top of the file, it gets updated upon saving.

  #+BEGIN_SRC emacs-lisp
(add-hook 'write-file-hooks 'time-stamp)
  #+END_SRC

- I ensure that in special modes (and calendar mode in particular) we don't display trailing whitespace, since they usually are read-only.

  #+BEGIN_SRC emacs-lisp
(add-hook 'special-mode-hook #'(lambda () (setq show-trailing-whitespace nil)))
(add-hook 'calendar-mode-hook #'(lambda () (setq show-trailing-whitespace nil)))
  #+END_SRC

** Config Auto-tangle & Daemon Restart

When saving any ~.org~ file inside ~~/jaspah/.emacs.d/~, tangle it so the
output in ~tangles/~ stays current.  In daemon mode, also save all open
buffers and restart the daemon via systemd — the shell wrapper's reconnect
loop re-attaches any open emacsclient frames.  Non-daemon Emacs just tangles;
the next fresh launch picks up the new output via ~j/config/load-conf-dot-org~.

  #+BEGIN_SRC emacs-lisp
(defun j/config/auto-tangle-and-restart ()
  "Tangle current file if it is a .org config in jaspah/.emacs.d/.
In daemon mode, save all buffers and restart the daemon via systemd."
  (when (and buffer-file-name
             (string-suffix-p ".org" buffer-file-name)
             (file-in-directory-p buffer-file-name
                                  (expand-file-name "~/jaspah/.emacs.d/")))
    (org-babel-tangle)
    (when (daemonp)
      ;; Save everything before systemd kills us — including scratch buffers
      ;; the user may not have explicitly saved.
      (save-some-buffers t)
      (start-process "j/emacs-restart" nil
                     "systemctl" "--user" "restart" "emacs.service"))))

(add-hook 'after-save-hook #'j/config/auto-tangle-and-restart)
  #+END_SRC
...
[[https://github.com/xenodium/agent-shell][agent-shell]] provides a native Emacs buffer interface for interacting with LLM agents via the [[https://agentclientprotocol.org/][Agent Client Protocol (ACP)]]. It supports Claude Code, Gemini CLI, and others, with transcript saving and file insertion.

~shell-maker~ and ~acp~ are pulled in as dependencies automatically.
...
Each fn takes (skill action) where action is \\='check, \\='install, or \\='update.")
  ...
  (defun j/agent/claude-code-list-skills ()
    "Return list of installed Claude Code plugin names (bare name@scope strings)."
    (when (executable-find "claude")
      (let ((plugin-re "\\([[:alnum:]._-]+@[[:alnum:]._-]+\\)")
            results)
        (dolist (line (split-string
                       (shell-command-to-string "claude plugins list")
                       "\n" t)
                      (nreverse results))
          (when (string-match plugin-re line)
            (push (match-string-no-properties 1 line) results))))))
    ...
    (pcase action
      ...
      ('install (when (executable-find "claude")
                  (= 0 (call-process-shell-command
                         (format "claude plugins install %s" skill) nil nil))))
      ('update  (when (executable-find "claude")
                  (= 0 (call-process-shell-command
                         (format "claude plugins update %s" skill) nil nil))))))

  (defun j/agent/sync-skills (&optional update)
    "Sync marketplace skills for all configured tools.
Prompts to install any missing skills.  Interactively, also prompts to update
...
:PROPERTIES:
:VISIBILITY: folded
:header-args:emacs-lisp: :tangle (concat (expand-file-name "~/jaspah/.emacs.d/tangles/tests/") (file-name-base) "-test.el")
:END:
...
In GUI and daemon mode, Emacs doesn't inherit the shell's ~$PATH~. [[https://github.com/purcell/exec-path-from-shell][This package]] fixes that by importing env vars from a login shell at startup. We use ~("-l")~ (non-interactive login shell) for faster startup — this requires PATH and other vars to be exported from a login shell profile (~/.bash_profile~ → ~.bashrc~).
...
Furthermore, whenever I enter ~prog-mode~ I want to enable ~display-line-numbers-mode~, and I also want to enable my custom [[*Utilities][j/infer-indentation-style]] function. I also set up emacs so that it highlights trailing whitespace and deletes any trailing whitespace that I have in my files; this keeps everything nice and neat :~)
...
*** coq
#+BEGIN_SRC emacs-lisp
(let ((command-output (shell-command-to-string "opam switch show")))
  (when (string-match-p "br" command-output)
    (load-file "~/dev/bluerock/proprietary/fm-workspace/dev/fmdev.el")))
#+END_SRC
...
(use-package proof-general
  ...
  (defun j/proof-layout-by-monitor-orientation (&rest _args)
    "Set Proof General's 3-window policy from the current monitor orientation.
Portrait monitors stack the script, goals, and response buffers in one
column (`vertical'); landscape monitors use the two-column `hybrid' layout."
    (setq proof-three-window-mode-policy
          (if (j/frame-portrait-p) 'vertical 'hybrid)))

  ;; PG's `vertical' policy stacks script/goals/response in even splits. On a
  ;; portrait monitor, reorder to goals (top), response (middle), script (bottom)
  ;; with even splits — response sits directly below goals, where Coq's own
  ;; `coq-optimise-resp-windows' expects it and fits it to content per command.
  ;; Goals/response are dedicated (PG's output windows); script stays editable.
  (defun j/proof-layout-portrait (&rest _args)
    "Reorder PG's portrait stack to goals/response/script; Coq fits the response."
    (when (and (j/frame-portrait-p)
      ...
      (let ((goalswin (selected-window)))
        (set-window-buffer goalswin proof-goals-buffer)
        (let ((respwin (split-window goalswin nil 'below)))
          (set-window-buffer respwin proof-response-buffer)
          (let ((scriptwin (split-window respwin nil 'below)))
            (set-window-buffer scriptwin proof-script-buffer)
            (set-window-dedicated-p goalswin proof-three-window-enable)
            (set-window-dedicated-p respwin proof-three-window-enable)
            (select-window scriptwin))))))

  ;; A Coq session loads `proof-site'/`pg-response', never `proof-general.el',
  ;; so use-package's `:config' (keyed on the `proof-general' feature) never
  ;; runs. Advise via `pg-response' — the feature that defines
  ;; `proof-layout-windows' — so the layout is chosen around every PG relayout.
  (with-eval-after-load 'pg-response
    (advice-add 'proof-layout-windows :before #'j/proof-layout-by-monitor-orientation)
    (advice-add 'proof-layout-windows :after #'j/proof-layout-portrait))
  :hook
  ;; Coq never calls `proof-layout-windows' itself, so (re)lay out when scripting
  ;; starts. The advice above sets policy/order first; Coq's own
  ;; `coq-optimise-resp-windows' then fits the response per command.
  (proof-activate-scripting . proof-layout-windows)
  :bind (("C-c M-a" . (lambda () (interactive) (insert "α")))
         ("C-c M-b" . (lambda () (interactive) (insert "β")))
         ("C-c M-d" . (lambda () (interactive) (insert "δ")))
         ("C-c M-g" . (lambda () (interactive) (insert "η")))
         ("C-c M-l" . (lambda () (interactive) (insert "λ"))))
  :custom
...
*Note:* ~nael-mode-map~ isn't exposed by ~nael-autoloads~.
...
#+BEGIN_SRC emacs-lisp
(j/config/load-conf-dot-org "~/jaspah/.emacs.d/j-dot-org.org")
#+END_SRC
...
** markdown-mode
This is pretty self-explanatory. I got my config from [[https://jblevins.org/projects/markdown-mode/][this website]].
...
(use-package multiple-cursors
  :bind (("C-. C-." . mc/edit-lines)  ; windowed
         ("C-c M-." . mc/edit-lines)  ; windowless
        ))
#+END_SRC
** pdf-tools

DocView is OK, but it isn't great (especially because the resolution gets messed up by converting every page to a ~.png~). Instead, I use [[https://github.com/politza/pdf-tools][pdf-tools]], which is a popular replacement.
...
(use-package pdf-tools
  ...
  ;; initialize
  (pdf-tools-install)
  ;; make PDFs open to fit the page
  (setq-default pdf-view-display-size 'fit-width)
  ;; enable regular searching
...
I use ~windmove~ to simplify reorganization of my buffers.
...
** xclip
When using `emacs -nw` the system clipboard is not automatically shared with the emacs yank-ring. We can use [[https://www.emacswiki.org/emacs/xclip.el][xclip.el]] to sync the two without impacting emacs' multiple kill/yank functionality (cf. [[https://unix.stackexchange.com/questions/6640/emacs-command-to-cut-or-copy-to-system-clipboard][stackexchange post]]).
...
(use-package xclip
  ...
  :config
    (when (eq system-type 'darwin)
      (warn "xclip package may not work properly on macOS. Consider using pbcopy/pbpaste alternatives instead."))
  :init (xclip-mode 1)
)
#+END_SRC
...
(use-package zone
  ...
  :init
  :config
  (zone-when-idle 60)                                         ;; ===| SCORE |===
  (customize-set-variable 'zone-programs                      ;;       -/5
                          '[;;zone-pgm-jitter                 ;;        2
                            zone-pgm-putz-with-case           ;;        4
                            ;;qzone-pgm-dissolve              ;;        2
                            zone-pgm-explode                  ;;        4
                            ;;zone-pgm-whack-chars            ;;        3
                            ;;zone-pgm-rotate                 ;;        3
                            ;;zone-pgm-rotate-LR-lockstep     ;;        3
                            ;;zone-pgm-rotate-RL-lockstep     ;;        3
                            ;;zone-pgm-rotate-LR-variable     ;;        3
                            ;;zone-pgm-rotate-RL-variable     ;;        3
                            zone-pgm-drip                     ;;        4
                            ;;zone-pgm-drip-fretfully         ;;        2
                            ;;zone-pgm-five-oclock-swan-dive  ;;        1
                            ;;zone-pgm-martini-swan-dive      ;;        1
                            zone-pgm-rat-race                 ;;        4
                            zone-pgm-paragraph-spaz           ;;        4
                            zone-pgm-stress                   ;;        4
                            zone-pgm-stress-destress          ;;        4
                            zone-pgm-random-life              ;;        5
                            ])
  (customize-set-variable 'zone-pgm-random-life-wait 1))
#+end_src
