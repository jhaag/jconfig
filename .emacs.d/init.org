...
;;; jbootstrap --- Efficiently/lazily autotangle & load ~/jconfig/init.org. -*- lexical-binding: t -*-
;;; Commentary:
...
When saving any ~.org~ file inside ~~/jconfig/.emacs.d/~, tangle it so the
...
(defun j/config/auto-tangle-and-restart ()
  "Tangle current file if it is a .org config in jconfig/.emacs.d/.
In daemon mode, save all buffers and restart the daemon via systemd."
  (when (and buffer-file-name
             (string-suffix-p ".org" buffer-file-name)
             (file-in-directory-p buffer-file-name
                                  (expand-file-name "~/jconfig/.emacs.d/")))
    (org-babel-tangle)
    (when (daemonp)
