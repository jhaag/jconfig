...
Below, I document the contents of this init.el file. However, these blocks are not tangled because this code is contained separately in init.el.
...
#+BEGIN_SRC emacs-lisp :tangle no
  (when (file-readable-p "~/.emacs.d/org-confs/init.org")
    (org-babel-load-file (expand-file-name "~/.emacs.d/org-confs/init.org")))
#+END_SRC
...
- I remove /beginner/ Emacs stuff that I don't want to worry about.
  ...
  #+BEGIN_SRC emacs-lisp
    (tool-bar-mode -1)
    (toggle-scroll-bar -1)
    (customize-set-variable 'initial-frame-alist '((fullscreen . maximized)))
...
- I try to use emacs as a server, so that it is simple and easy to startup a new session, and so that I can save my yank-ring and other stuff between sessions.

  #+BEGIN_SRC emacs-lisp
    (server-start)
  #+END_SRC

- Highlight trailing whitespace in red so that it's easily visible.

  #+BEGIN_SRC emacs-lisp
    (customize-set-variable 'show-trailing-whitespace t)
  #+END_SRC

- Even though I use ~org-mode~, I also want to be able to keep track of changes on my other files. I set up emacs to timestamp every file at the top upon save. I also set up emacs so that it deletes any trailing whitespace that I have in my files; this keeps everything nice and neat :~)
...
I find it quite annoying how Emacs keeps backups locally, and so I wanted to create a nicer way to handle that. All that I do is store my backups in a sub-folder of my main ~.emacs.d~ folder.
...
I also wanted to make sure that I never keep too many backups, and so I have a bit of code which manually crawls my backup folder and deletes anything which is over 1 month old.
...
Now we can load git-gutter+ itself, and configure it to our liking.
...
We also require ~git-gutter-fringe+~, which is the actual package that allows us to display our git status in the buffer.
...
#+BEGIN_SRC emacs-lisp
  (use-package git-gutter-fringe+
    ...
    (fring-helper-define 'git-gutter-fr+-modified nil
                         "........"
                         "........"
                         "..XX...X"
                         ".XXXX.XX"
...
Furthermore, whenever I enter ~prog-mode~ I want to enable ~linum-mode~, and I also want to enable my custom [[*Utilities][j/infer-indentation-style]] function.
...
#+BEGIN_SRC emacs-lisp
  (when (file-readable-p "~/jconfig/.emacs.d/org-confs/j-dot-org.org")
    (org-babel-load-file "~/jconfig/.emacs.d/org-confs/j-dot-org.org"))
    ;;(use-package j-dot-org
    ;;  :commands j/org-startup
    ;;  ;; I'm not exactly sure why I need a :bind* here instead of :bind, but jwiegley had it, so I do
